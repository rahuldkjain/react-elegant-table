...
const columns = [
  {
    header: "Image",
    accessorFn: (data: LooseObject) => {
      return <img role="img" alt={data.title} src={data.thumbnail} />;
    },
    width: "15%",
  },
  {
...
function customSort<T extends Record<string, any>>(
  list: T[],
  key: string,
  order: "asc" | "desc" = "asc"
): T[] {
  return [...list].sort((a, b) => {
    ...
    // Use localeCompare to handle alphanumeric sorting
...
const PaginatedStaticTable = () => {
  ...
  const handleDeleteSelectedItems = () => {
    const selectedRowKeys = Object.keys(selection);
    const newRows = [...rows].filter((row) => {
      let value: string | number = row[ROW_KEY];
      if (typeof value === "number") {
        value = value.toString();
      }
      return selectedRowKeys.includes(value) ? false : true;
    });
    setRows(newRows);
    setSelection({});
  };
  return (
    <Table
      showAlternateRowColored
      header={{
        title: "Preview paginated static table",
        subtitle:
          "this table supports sorting on id & title column, pagination, search, filter and other actions",
        infoText: "10 items",
        actions: (
          <button className="px-4 border border-black text-white bg-black rounded h-10">
            Add item
          </button>
        ),
      }}
      actions={{
        ...
        otherActions: (
          <button
            className={clsx(
              "bg-white-400 h-10 px-4 rounded bg-red-500 text-white",
              {
      ...
      pagination={{
        pageSize: PAGE_SIZE,
        totalPageCount: Math.ceil(DEFAULT_ROWS.length / PAGE_SIZE),
      }}
      onSort={(sortKey: string, type) => {
        let sortedRows = customSort([...rows], sortKey, type);
        setRows(sortedRows);
      }}
    />
  );
